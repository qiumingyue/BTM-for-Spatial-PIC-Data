...
function loglikelihood(beta,gamma,spatial,r)
  LambdaL = bisL' * gamma
  LambdaR = bisR' * gamma
...
#function est(r, order_1, n_knots, niter, a_eta, b_eta, a_lambda, b_lambda, data)
  ...
  for i = 1001:6000
    likelihood_iter[i-1000,:] = loglikelihood(parbeta[i,:],pargam[i,:],parspatial_all[i,:],r)
...
exact_rate = 0.2
true_beta = [0.5,1]
cd("E:\\Spatial\\transformation model with spatial")
W = Matrix(CSV.read("matrix.csv",DataFrame))
tau = 1
r = 0
#data = Data_generation(123,N_S,exact_rate,true_beta,fun,r,W,tau)
n_knots = 10
order_1 = 2
a_eta = b_eta = 1
a_tau = b_tau = 0.1
niter = 11000
global sigma = 1
global MH_iter = 11000
global c0 = 2
global MH_iter_spatial = 11000
global c1 = 2
global g = 1
allnum = 10
right_censored_rate = zeros(allnum)
test_estimator = zeros(allnum,length(true_beta))
test_ess = zeros(allnum,length(true_beta))
test_lower = zeros(allnum,length(true_beta))
test_upper = zeros(allnum,length(true_beta))
test_mcse = zeros(allnum,length(true_beta))
test_parvar = zeros(allnum,length(true_beta))
test_tau = zeros(allnum)
n = N_S * size(W)[1]
for ind = 1:allnum
  data = Data_generation(ind,N_S,exact_rate,true_beta,fun,r,W,tau)
  right_censored_rate[ind] = sum(data[:,(size(data)[2]-1)] .== repeat([3.0],n))/n #右删失个数
  result = est(r, order_1, n_knots, niter, a_eta, b_eta, a_tau, b_tau, data)
  test_estimator[ind,:] = result["mean"]
  test_ess[ind,:] = result["ess"]
  test_lower[ind,:] = result["lower"]
  test_upper[ind,:] = result["upper"]
  test_mcse[ind,:] = result["mcse"]
  test_parvar[ind,:] = result["parvar"]
  test_tau[ind] = result["tau"]
end
